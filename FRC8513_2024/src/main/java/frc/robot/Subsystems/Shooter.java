...
public class Shooter {
    ...
    PIDController leftShooterPIDController = new PIDController(Settings.shooter_P, Settings.shooter_I,
            Settings.shooter_D);
    PIDController rightShooterPIDController = new PIDController(Settings.shooter_P, Settings.shooter_I,
            Settings.shooter_D);
    ...
    public void applyShooterPower() {
        ...
        if (leftShooterGoalSpeed == 0) {
            ...
            double leftShooterPower = leftShooterPIDController.calculate(leftShooter.getEncoder().getVelocity(),
                    leftShooterGoalSpeed);
        ...
        if (rightShooterGoalSpeed == 0) {
            ...
            double rightShooterPower = rightShooterPIDController.calculate(rightShooter.getEncoder().getVelocity(),
                    rightShooterGoalSpeed);
    ...
    public boolean rightShooterInThreshold() {
        return Math
                .abs(rightShooter.getEncoder().getVelocity() - rightShooterGoalSpeed) < Settings.shooterThresholdValue;
    }
    ...
    public double getDistFromGoal() {
        ...
        if (thisRobot.onRedAlliance) {
            temp = thisRobot.drivebase.swerveDrive.getPose().minus(new Pose2d(Settings.redGoalPos, new Rotation2d()))
                    .getTranslation().getNorm();

        } else {
            temp = thisRobot.drivebase.swerveDrive.getPose().minus(new Pose2d(Settings.blueGoalPos, new Rotation2d()))
                    .getTranslation().getNorm();
        }
        return temp;
    }
